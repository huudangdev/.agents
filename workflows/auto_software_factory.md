...
Hệ thống V29.0 nghiêm cấm Load cùng lúc >10 File kỹ năng. Mỗi khi bước sang STAGE mới, Antigravity **BẮT BUỘC** gọi Tool `view_file` đọc file `SKILLS_INDEX.md`, tra cứu đúng thư mục chứa tên Role đó (VD: search `maya`), và nạp file `.agents/skills/maya/SKILL.md`.  
...
*🧠 IDENTITY INJECTION: Dùng `view_file` đọc `.agents/skills/elite6-research/SKILL.md`, `sophia/SKILL.md`, `antigravity-brainstorming/SKILL.md`, `product-brainstorming/SKILL.md` và `compound-brainstorming/SKILL.md`*
...
*🧠 IDENTITY INJECTION: Dùng `view_file` đọc `.agents/skills/maya/SKILL.md` và `.agents/skills/design-system-rules/SKILL.md`*
...
*🧠 IDENTITY INJECTION: Dùng `view_file` đọc các skills: `david`, `chartis`, `architecture-decision-records`, `c4-architecture`, `claude-arch-designer`, `knowledge-work-architecture`, `software-architecture`, `langchain-architecture`.*
*📦 OUTPUT GENERATED: `SDD_*.md`, `UML/*.md`, `PDF/*.pdf`*
**[Step 15-17]**: Architect David & Data Leo (`leo/SKILL.md`) viết Hệ tầng Backend (FSD, Database Schema, API Contracts). Nhúng Sơ Đồ Cấu Trúc Trực Tiếp vào Dòng md. Đọc bắt buộc `architecture.md` (nếu có). Đi vào sâu các nguyên tắc từ `.agents/skills/architecture-patterns`.
...
*🧠 IDENTITY INJECTION: Dùng `view_file` đọc `.agents/skills/eve/SKILL.md`, `alan/SKILL.md`, và `.agents/skills/mobile-app-testing/SKILL.md`*
...
*🧠 IDENTITY INJECTION: Dùng `view_file` ĐỌC `.agents/skills/benny/SKILL.md` và MỘT CÁCH TÀN ĐỘC LÀ ĐỌC THÊM `.agents/skills/qa-simulator/SKILL.md` (Hoặc tra `SKILLS_INDEX.md` nạp skill Frontend tương đương).*
...
*🧠 IDENTITY INJECTION: Dùng `view_file` đọc `.agents/skills/ops/SKILL.md`*
...
*🧠 IDENTITY INJECTION: Dùng `view_file` đọc `.agents/skills/sophia/SKILL.md` (Butler Mode)*
