...
**GIAO THỨC NHẬP HỒN VẬT LÝ (NATIVE IDENTITY INJECTION):**
Mỗi khi bước sang một STAGE mới, Antigravity **TUYỆT ĐỐI BẮT BUỘC** dùng Tool `view_file` để **MỞ VÀ ĐỌC CHI TIẾT** các file nằm trong khối `[🧠 IDENTITY INJECTION]`. Đoán mò tính cách = Lỗi Logic.
</identity_binding_protocol>
...
*🧠 IDENTITY INJECTION: Dùng `view_file` đọc `.agents/skills/maya/SKILL.md`*
...
*🧠 IDENTITY INJECTION: Dùng `view_file` đọc `.agents/skills/david/SKILL.md` và `chartis/SKILL.md`*
...
*🧠 IDENTITY INJECTION: Dùng `view_file` đọc `.agents/skills/eve/SKILL.md` và `alan/SKILL.md`*
...
*🧠 IDENTITY INJECTION: Dùng `view_file` ĐỌC `.agents/skills/benny/SKILL.md` và MỘT CÁCH TÀN ĐỘC LÀ ĐỌC THÊM `.agents/skills/qa-simulator/SKILL.md`*
...
MỘT GIAO THỨC HAI PHA BẮT BUỘC CHO TỪNG BUTTON VÀ PAGE:
  ...
  - Bắt buộc Coder sửa mượt mà 100% màn hình hiện tại trên Môi trường chạy Live này, rồi TÍCH XANH, sau đó mới cho qua Screen kề tiếp MAPPING trong PRD!
...
**[Step 48-52]**: DevOps Release Cửa Cuối -> Bắn Ping End-To-End (Playwright) -> LỖI ĐÂU SỬA ĐÓ! Systematic Debugging -> Tắt máy chủ ngầm -> Báo Cáo Người Dùng.
