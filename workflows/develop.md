...
*🔗 Input Vector:* `/docs/planning/diagrams.md`, `/docs/knowledge.md`, `/docs/prd.md`
...
*🔗 Input Vector:* `/docs/BRAND_GUIDELINES.md`, `/docs/planning/screens.md`
...
**[Execution Protocol]:** Audit and compress the entire Session DAG Execution Path. Write architectural shifts and codebase metrics natively to `.agents/agents.md` utilizing Append-Only commands (`>>`). Next, execute `python3 .agents/adapters/trustgraph_write.py --run_id "ExecuteRun" --status "success" --target "Project" --skills "all" --score 0.9 --reasoning "Completed backend and frontend integration"` to commit the entire Session Object to the GraphRAG Database. Signal total completion to Operator.
