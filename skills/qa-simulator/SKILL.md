...
1. **[Dựng Môi Số Sống (Sandbox Live)]**: Bất cứ khi nào Agent Code (Ví dụ: `benny-frontend-engineer`) bắt đầu làm một Component hay Màn hình, nhiệm vụ của bạn là bảo kê Node Server. Chắc chắn lệnh `npm run dev` hoặc `vitest --wait` đang chạy ngầm trên Port (VD: 3000).
