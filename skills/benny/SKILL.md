...
**BẢNG MÃ THIẾT KẾ RẬP KHUÔN (DESIGN CHEATSHEET - CẤM SAI LỆCH):**
- **Typography:** Chỉ dùng 2 size: 16px & 14px, regular + medium.
- **Spacing:** Hệ số 4px, không có ngoại lệ.
- **Buttons:** Chiều cao 40px min-height.
- **Radius:** Dao động 8px -> 40px.
- **Icons:** Box size 24px, stroke 1.5 -> 2px.
...
---
**[V28 THE AGENTIC DESIGN PATTERNS - BỔ SUNG CODE EXECUTION ĐỈNH CAO UI]**
Để Code Frontend cho Hệ thống AI đạt tính Tương Tác Cấp Thần Phẩm (Agentic Generative UI) như PromptAdvisers yêu cầu:

1. **Dynamic Streaming & Parsing JSON Engine:**
   - Kết nối dữ liệu dạng Streaming (Server-Sent Events / Async Generators). Cấm code component chỉ nhận 1 cục Data JSON tĩnh đợi cả phút.
   - Render Dữ liệu Markdown theo thời gian thực (VD: `react-markdown` + `remark-gfm`).

2. **Render Tuyệt Đối Các Agentic States:**
   - Khi Code Component giao tiếp với AI, chia rạch ròi State: `isThinking`, `isStreaming`, `awaitingApproval`. 
   - Skeleton Shimmer tĩnh thay thế Spinner Loading cho `isThinking`. Các Text động (Framer Motion Ghost-Text) "System is resolving reasoning vectors...".

3. **Human-In-The-Loop (HITL) Implementation:**
   - Bất kỳ AI Generated Output nào cũng gắn liền với Action Menu: `onRegenerate()`, `onModify(prompt)`, `onApprove(payload)`, `onDeny()`.
   - Micro-interaction: `stiffness=400, damping=30`.
